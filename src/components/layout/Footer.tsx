 ...
 * with About and Contact, the column someone who wants to
...
export function Footer() {
  ...
  return (
    <footer className="relative overflow-hidden bg-navy text-ink-300">
      <Container className="relative">
        <div className="grid items-start gap-10 border-b border-white/15 py-9 lg:grid-cols-[17rem_1fr] lg:gap-12">
          <div>
            <Logo onDark />
            <p className="mt-4 max-w-[17rem] text-[0.8125rem] leading-relaxed text-ink-200">
              BIM automation plugins for Architecture, Structure and MEP — from early design
              to as‑built.
            </p>
          </div>
                ...
                <ul className="space-y-2.5">
                  {column.links.map((link) => (
                    <li key={link.label + link.href}>
                      <Link
                        href={link.href}
                        className="text-[0.8125rem] leading-5 text-ink-200 transition-colors hover:text-brand-300"
                      >
                        {link.label}
                      </Link>
                    </li>
        ...
        <div className="flex flex-col items-start justify-between gap-3 py-4 sm:flex-row sm:items-center">
          ...
          <div className="flex flex-wrap items-center gap-x-5 gap-y-2">
            <Link
              href="/privacy-policy"
              className="text-2xs text-ink-300 transition-colors hover:text-white"
            >
              Privacy Policy
            </Link>
            <Link

 ...
 */
export const attrs = ["stroke-dashoffset"];
...
export function values(p) {
  ...
  phases([[0,20],[20,38],[38,58],[58,80]]);
  var L = [330, 200, 337, 160], st = [4, 22, 40, 60], dur = [14, 12, 14, 12];
  for (var i = 0; i < 4; i++) { v['o'+i] = (L[i] * (1 - lin([[st[i],0],[st[i]+dur[i],1]]))).toFixed(1); v['k'+i] = p >= st[i] + dur[i] ? out : 0.25 * out; }
  v.tm = lin([[70,0],[74,1]]) * out;
